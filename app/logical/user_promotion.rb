class UserPromotion
  attr_reader :user, :promoter, :new_level, :options, :old_can_approve_posts, :old_can_upload_free, :old_no_flagging, :old_replacements_beta, :old_is_bd_staff
  ...
  def promote!
    ...
    @old_is_bd_staff = user.is_bd_staff?
    ...
    if options.key?(:is_bd_staff)
      user.is_bd_staff = options[:is_bd_staff]
    end
  ...
  def create_mod_actions
    ...
    flag_check(added, removed, "is_bd_staff", "is bd staff")
  ...
  def validate
    ...
    raise User::PrivilegeError, "Only Owner can promote to BD Staff" if options.key?(:is_bd_staff) && options[:is_bd_staff] && promoter.level < User::Levels::OWNER
  end
end
